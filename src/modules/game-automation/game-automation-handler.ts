...
export class FO76AutomationHandler {
  ...
  init = () => {
    console.log('FO76AutomationHandler', 'Initializing game automation handler')
  ...
  startCountdown(mode: string): void {
    ...
    const logIntervalId = setInterval((): void => {
      if (config.isRunning) {
        console.log('FO76AutomationHandler', `Loop ${mode} will run again in ${countdown} seconds`)
  ...
  private isConfigChanged(mode: FO76AutomationMode, existingConfig: LoopConfig): boolean {
    ...
    console.log('FO76AutomationHandler', `Is config changed for ${mode.name}: `, isConfigChanged)
  ...
  handleFO76AutomationEvent = async (state: {
    ...
    try {
      ...
      if (mode && mode.name) {
        ...
        console.log('FO76AutomationHandler', 'No mode provided, stopping all modes')
      }
  ...
  incrementDecrementDefaultInterval = async (increment?: boolean, decrement?: boolean) => {
    ...
    console.log('FO76AutomationHandler', `Incrementing default interval to ${newInterval}`)
