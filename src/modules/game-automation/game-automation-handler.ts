 ...
 * @description Handles the game automation functionality for Fallout 76
 */
export class FO76AutomationHandler {
  ...
  init = () => {
    BxLogger.info('FO76AutomationHandler', 'Initializing game automation handler')
  ...
  private initializeDefaultConfigs() {
    const defaultConfigs = [
      ...
      },
      {
        ...
        action: async () => {
          await this.pressButtonWithRandomDelay(GamepadKey.RS_RIGHT, this.DEFAULT_ACTION_INTERVAL)
          await this.delay(500)
          await this.pressButtonWithRandomDelay(GamepadKey.RS_LEFT, this.DEFAULT_ACTION_INTERVAL)
        },
      },
      {
  ...
  private startButtonLoop(mode: string): void {
    Toast.show('Game FO76Automation', `Starting ${mode} loop`, { instant: true })
    this.setButtonLoop(mode, true)
  }

  private stopButtonLoop(mode: string): void {
    Toast.show('Game FO76Automation', `Stopping ${mode} loop`, { instant: true })
  ...
  startCountdown(mode: string): void {
    ...
    const logIntervalId = setInterval((): void => {
      if (config.isRunning) {
        BxLogger.info('FO76AutomationHandler', `Loop ${mode} will run again in ${countdown} seconds`)
        Toast.show('Game FO76Automation', `${mode} will run again in ${countdown} seconds`, {
          instant: true,
        })
        countdown--
        if (countdown < 0) {
  ...
  toggle = () => {
    BxLogger.info('FO76AutomationHandler', 'Toggling game automation')
  ...
  activate = () => {
    ...
    BxLogger.info('FO76AutomationHandler', 'Activating game automation')
    this.enabled = true
  ...
  deactivate = () => {
    ...
    BxLogger.info('FO76AutomationHandler', 'Deactivating game automation')
    this.enabled = false
  ...
  handleWindowBlur = () => {
    ...
    BxLogger.info('FO76AutomationHandler', 'Window blurred')
  }
  ...
  handleWindowFocus = () => {
    ...
    BxLogger.info('FO76AutomationHandler', 'Window focused')
  }
  ...
  private isConfigChanged(mode: FO76AutomationMode, existingConfig: LoopConfig): boolean {
    const changeLog = {
      actionInterval: {
    ...
    BxLogger.info('FO76AutomationHandler', `Change log for ${mode.name}: `, changeLog)

    const isConfigChanged = Object.values(changeLog).some((value) => {
      return value.changed
    })

    BxLogger.info('FO76AutomationHandler', `Is config changed for ${mode.name}: `, isConfigChanged)
    ...
    try {
      ...
      if (mode && mode.name) {
        ...
        if (!existingConfig || !existingConfig.isRunning) {
          BxLogger.info('FO76AutomationHandler', `Starting mode ${mode.name}`)
          this.updateMode(mode.name, {
          ...
          BxLogger.info('FO76AutomationHandler', `Stopping mode ${mode.name}`)
          ...
          if (!this.isConfigChanged(mode, existingConfig)) {
            ...
            BxLogger.info('FO76AutomationHandler', `Updating mode ${mode.name}`)
            this.updateMode(mode.name, {
              actionInterval: mode.interval || existingConfig.actionInterval,
              pauseDuration: mode.pause || existingConfig.pauseDuration,
              action: existingConfig.action,
        ...
        BxLogger.info('FO76AutomationHandler', 'No mode provided, stopping all modes')
      }
      ...
      BxLogger.error('FO76AutomationHandler', 'Failed to handle automation event', error)
      return {
        status: 'error',
  ...
  toggleFO76Automation = async (
    ...
    try {
      ...
      BxLogger.error('FO76AutomationHandler', 'Failed to toggle automation', error)
      return {
        status: 'error',
        automationActive: this.enabled,
      }
  ...
  sendDefaultEvent = async (modeName: FO76AutomationModes, interval: number, pause: number) => {
    try {
      const mode = {
        name: modeName,
        interval,
  ...
  incrementDecrementDefaultInterval = async (increment?: boolean, decrement?: boolean) => {
    const newInterval = this.DEFAULT_ACTION_INTERVAL + (increment ? 1000 : decrement ? -1000 : 0)
    if (newInterval < 1000) {
    ...
    BxLogger.info('FO76AutomationHandler', `Incrementing default interval to ${newInterval}`)
...
class EventManager {
  static registerEventListeners(handler: FO76AutomationHandler) {
    ...
    window.addEventListener(FO76_AUTOMATION_EVENTS.TOGGLE_MODE, handler.handleToggleModeEvent)
    window.addEventListener(
...
class LoopManager {
  ...
  startLoop(config: LoopConfig, mode: string, handler: FO76AutomationHandler) {
    ...
    const loopFunction = async (): Promise<void> => {
      ...
      BxLogger.info('FO76AutomationHandler', `Running ${mode} loop`)
      await config.action()
