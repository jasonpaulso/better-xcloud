...
import { FO76_AUTOMATION_EVENTS } from "./game-automation/game-automation-handler";
...
const enum ShortcutAction {
  ...
  TOGGLE_AWAY_MODE_HEAL = "toggle-away-mode-heal",
...
export class ControllerShortcut {
  private static readonly STORAGE_KEY = StorageKey.CONTROLLER_SHORTCUTS;
  ...
  private static $selectActions: Partial<{
    [key in GamepadKey]: HTMLSelectElement;
  }> = {};
  private static $container: HTMLElement;

  private static ACTIONS: { [key: string]: (ShortcutAction | null)[] } | null =
    null;
  ...
  private static runAction(action: ShortcutAction) {
    switch (action) {
      ...
      case ShortcutAction.TOGGLE_AWAY_MODE_AWAY:
        BxEvent.dispatch(window, FO76_AUTOMATION_EVENTS.TOGGLE_MODE, {
          detail: { name: "away", toggle: true },
        });

        break;

      case ShortcutAction.TOGGLE_AWAY_MODE_VATS:
        BxEvent.dispatch(window, FO76_AUTOMATION_EVENTS.TOGGLE_MODE, {
          detail: { name: "vats", toggle: true },
        });

        break;
      case ShortcutAction.INCREMENT_AWAY_MODE_INTERVAL:
        BxEvent.dispatch(window, FO76_AUTOMATION_EVENTS.CHANGE_INTERVAL, {
          detail: { increment: true, source: "controller" },
        });
        break;

      case ShortcutAction.DECREMENT_AWAY_MODE_INTERVAL:
        BxEvent.dispatch(window, FO76_AUTOMATION_EVENTS.CHANGE_INTERVAL, {
  ...
  private static updateAction(
    ...
    window.localStorage.setItem(
      ControllerShortcut.STORAGE_KEY,
      JSON.stringify(ControllerShortcut.ACTIONS)
    );
  }
  ...
  private static updateProfileList(e?: GamepadEvent) {
    ...
    for (const gamepad of gamepads) {
      ...
      const $option = CE<HTMLOptionElement>(
        "option",
        { value: gamepad.id },
        gamepad.id
      );
  ...
  private static getActionsFromStorage() {
    return JSON.parse(
      window.localStorage.getItem(ControllerShortcut.STORAGE_KEY) || "{}"
    );
  }
  ...
  static renderSettings() {
    ...
    const actions: {
      [key: string]: Partial<{ [key in ShortcutAction]: string | string[] }>;
    } = {
      [t("better-xcloud")]: {
        [ShortcutAction.BETTER_XCLOUD_SETTINGS_SHOW]: [
          t("settings"),
          t("show"),
        ],
        [ShortcutAction.TOGGLE_AWAY]: ["Away Mode - Toggle", t("toggle")],
        [ShortcutAction.TOGGLE_AWAY_MODE_HEAL]: [
          "Away Mode — Heal",
          t("toggle"),
        ],
        [ShortcutAction.TOGGLE_AWAY_MODE_PIVOT]: [
          "Away Mode — Pivot",
          t("toggle"),
        ],
        [ShortcutAction.TOGGLE_AWAY_MODE_AWAY]: [
          "Away Mode — Away",
          t("toggle"),
        ],
        [ShortcutAction.TOGGLE_AWAY_MODE_VATS]: [
          "Away Mode — Vats",
          t("toggle"),
        ],
        [ShortcutAction.INCREMENT_AWAY_MODE_INTERVAL]: [
          "Away Mode - Interval",
          t("increase"),
        ],
        [ShortcutAction.DECREMENT_AWAY_MODE_INTERVAL]: [
          "Away Mode - Interval",
          t("decrease"),
        ],
        [ShortcutAction.TOGGLE_AWAY_MODE_INTERACT]: [
          "Away Mode — Interact",
          t("toggle"),
        ],
      },
      ...
      [t("device")]: AppInterface && {
        ...
        [ShortcutAction.DEVICE_BRIGHTNESS_INC]: [
          t("brightness"),
          t("increase"),
        ],
        [ShortcutAction.DEVICE_BRIGHTNESS_DEC]: [
          t("brightness"),
          t("decrease"),
        ],
      },
      ...
      [t("stream")]: {
        ...
        [ShortcutAction.STREAM_VOLUME_INC]: getPref(
          PrefKey.AUDIO_ENABLE_VOLUME_CONTROL
        ) && [t("volume"), t("increase")],
        [ShortcutAction.STREAM_VOLUME_DEC]: getPref(
          PrefKey.AUDIO_ENABLE_VOLUME_CONTROL
        ) && [t("volume"), t("decrease")],

        [ShortcutAction.STREAM_MENU_SHOW]: [t("menu"), t("show")],
        [ShortcutAction.STREAM_STATS_TOGGLE]: [t("stats"), t("show-hide")],
        [ShortcutAction.STREAM_MICROPHONE_TOGGLE]: [
          t("microphone"),
          t("toggle"),
        ],
      },
    ...
    const $baseSelect = CE<HTMLSelectElement>(
      "select",
      { autocomplete: "off" },
      CE("option", { value: "" }, "---")
    );
    for (const groupLabel in actions) {
      ...
      const $optGroup = CE<HTMLOptGroupElement>("optgroup", {
        label: groupLabel,
      });
    ...
    const $selectProfile = CE<HTMLSelectElement>("select", {
      class: "bx-shortcut-profile",
      autocomplete: "off",
    });

    const $profile = PREF_CONTROLLER_FRIENDLY_UI
      ? BxSelectElement.wrap($selectProfile)
      : $selectProfile;
    ...
    const $container = CE(
      ...
      CE(
        ...
        CE(
          ...
          t("controller-shortcuts-connect-note")
        )
      ),
    ...
    const onActionChanged = (e: Event) => {
      ...
      if (!PREF_CONTROLLER_FRIENDLY_UI) {
        const $fakeSelect =
          $target.previousElementSibling! as HTMLSelectElement;
        let fakeText = "---";
        if (action) {
    ...
    for (const [button, prompt] of buttons) {
      ...
      const $label = CE(
        "label",
        { class: "bx-prompt" },
        `${PrompFont.HOME} + ${prompt}`
      );

      const $div = CE("div", { class: "bx-shortcut-actions" });

      if (!PREF_CONTROLLER_FRIENDLY_UI) {
        const $fakeSelect = CE<HTMLSelectElement>(
          "select",
          { autocomplete: "off" },
          CE("option", {}, "---")
        );
