...
function getSupportedCodecProfiles() {
  ...
  if (hasLowCodec) {
    if (!hasNormalCodec && !hasHighCodec) {
      options[CodecProfile.DEFAULT] = `${t("visual-quality-low")} (${t(
        "default"
      )})`;
    } else {
  ...
  if (hasNormalCodec) {
    if (!hasLowCodec && !hasHighCodec) {
      options[CodecProfile.DEFAULT] = `${t("visual-quality-normal")} (${t(
        "default"
      )})`;
    } else {
  ...
  if (hasHighCodec) {
    if (!hasLowCodec && !hasNormalCodec) {
      options[CodecProfile.DEFAULT] = `${t("visual-quality-high")} (${t(
        "default"
      )})`;
    } else {
  ...
  private static readonly DEFINITIONS = {
    ...
    [PrefKey.CONTROLLER_DEVICE_VIBRATION]: {
      ...
      options: {
        ...
        [ControllerDeviceVibration.AUTO]: t(
    ...
    [PrefKey.STATS_ITEMS]: {
      ...
      multipleOptions: {
        ...
        [StreamStat.PLAYTIME]: `${StreamStat.PLAYTIME.toUpperCase()}: ${t(
          "playtime"
        )}`,
        [StreamStat.BATTERY]: `${StreamStat.BATTERY.toUpperCase()}: ${t(
          "battery"
        )}`,
        [StreamStat.PING]: `${StreamStat.PING.toUpperCase()}: ${t(
          "stat-ping"
        )}`,
        [StreamStat.JITTER]: `${StreamStat.JITTER.toUpperCase()}: ${t(
          "jitter"
        )}`,
        [StreamStat.FPS]: `${StreamStat.FPS.toUpperCase()}: ${t("stat-fps")}`,
        [StreamStat.BITRATE]: `${StreamStat.BITRATE.toUpperCase()}: ${t(
          "stat-bitrate"
        )}`,
        [StreamStat.DECODE_TIME]: `${StreamStat.DECODE_TIME.toUpperCase()}: ${t(
          "stat-decode-time"
        )}`,
        [StreamStat.PACKETS_LOST]: `${StreamStat.PACKETS_LOST.toUpperCase()}: ${t(
          "stat-packets-lost"
        )}`,
        [StreamStat.FRAMES_LOST]: `${StreamStat.FRAMES_LOST.toUpperCase()}: ${t(
          "stat-frames-lost"
        )}`,
        [StreamStat.DOWNLOAD]: `${StreamStat.DOWNLOAD.toUpperCase()}: ${t(
          "downloaded"
        )}`,
        [StreamStat.UPLOAD]: `${StreamStat.UPLOAD.toUpperCase()}: ${t(
          "uploaded"
        )}`,
      },
      params: {
    ...
    [PrefKey.PREFERENCES_FOLDER_PATH]: {
      label: t("preferences-folder-path"),
      default: "",
      note: t("preferences-folder-path-note"),
    },

    [PrefKey.PREFERENCES_FOLDER_ENABLED]: {
      label: t("preferences-folder-enabled"),
      default: false,
      note: t("preferences-folder-enabled-note"),
    },
...
export const getPrefDefinition =
  globalSettings.getDefinition.bind(globalSettings);
