...
export class Translations {
  ...
  static async init() {
    Translations.#enUsIndex = Translations.#supportedLocales.indexOf(
      Translations.#EN_US
    );
  ...
  static async downloadTranslations(locale: string) {
    try {
      const resp = await NATIVE_FETCH(
        `https://raw.githubusercontent.com/redphx/better-xcloud/gh-pages/translations/${locale}.json`
      );
