...
var SCRIPT_VERSION = "x.x.x", AppInterface = window.AppInterface;
UserAgent.init();
...
class StreamStats {
  ...
  async#update() {
    ...
    this.#timeoutId = null;
    let startTime = performance.now(), PREF_STATS_CONDITIONAL_FORMATTING = getPref("stats_conditional_formatting"), stats = await STATES.currentStream.peerConnection.getStats(), grade = "";
    stats.forEach((stat) => {
      if (stat.type === "inbound-rtp" && stat.kind === "video") {
        this.#$fps.textContent = stat.framesPerSecond || 0;
        let packetsLost = Math.max(0, stat.packetsLost), packetsReceived = stat.packetsReceived, packetsLostPercentage = (packetsLost * 100 / (packetsLost + packetsReceived || 1)).toFixed(2);
        this.#$pl.textContent = packetsLostPercentage === "0.00" ? packetsLost.toString() : `${packetsLost} (${packetsLostPercentage}%)`;
        let { framesDropped, framesReceived } = stat, framesDroppedPercentage = (framesDropped * 100 / (framesDropped + framesReceived || 1)).toFixed(2);
        ...
        let lastStat = this.#lastVideoStat, timeDiff = stat.timestamp - lastStat.timestamp, bitrate = 8 * (stat.bytesReceived - lastStat.bytesReceived) / timeDiff / 1000;
        this.#$br.textContent = `${bitrate.toFixed(2)} Mbps`;
        let totalDecodeTimeDiff = stat.totalDecodeTime - lastStat.totalDecodeTime, framesDecodedDiff = stat.framesDecoded - lastStat.framesDecoded, currentDecodeTime = totalDecodeTimeDiff / framesDecodedDiff * 1000;
        ...
        let roundTripTime = stat.currentRoundTripTime ? stat.currentRoundTripTime * 1000 : -1;
    ...
    let lapsedTime = performance.now() - startTime;
    this.#timeoutId = window.setTimeout(this.#update.bind(this), this.#updateInterval - lapsedTime);
  }
  refreshStyles() {
    let PREF_ITEMS = getPref("stats_items"), PREF_POSITION = getPref("stats_position"), PREF_TRANSPARENT = getPref("stats_transparent"), PREF_OPACITY = getPref("stats_opacity"), PREF_TEXT_SIZE = getPref("stats_text_size"), $container = this.#$container;
  ...
  #render() {
    ...
    }, $barFragment = document.createDocumentFragment(), statKey;
...
class GlobalSettingsStorage extends BaseSettingsStore {
  static DEFINITIONS = {
    ...
    mkb_enabled: {
      ...
      default: !1,
      unsupported: (() => {
        let userAgent2 = (window.navigator.orgUserAgent || window.navigator.userAgent || "").toLowerCase();
...
function onChangeVideoPlayerType() {
  let playerType = getPref("video_player_type"), $videoProcessing = document.getElementById("bx_setting_video_processing"), $videoSharpness = document.getElementById("bx_setting_video_sharpness"), $videoPowerPreference = document.getElementById("bx_setting_video_power_preference");
  if (!$videoProcessing)
    return;
...
var true_achievements_default = `<svg xmlns='http://www.w3.org/2000/svg' fill='#fff' stroke='nons' fill-rule='evenodd' stroke-linecap='round' stroke-linejoin='round' stroke-width='2' viewBox='0 0 32 32'>
...
var power_default = `<svg xmlns='http://www.w3.org/2000/svg' fill='none' stroke='#fff' fill-rule='evenodd' stroke-linecap='round' stroke-linejoin='round' stroke-width='2' viewBox='0 0 32 32'>
...
var speaker_slash_default = `<svg xmlns='http://www.w3.org/2000/svg' fill='#fff' stroke='none' fill-rule='evenodd' stroke-linecap='round' stroke-linejoin='round' stroke-width='2' viewBox='0 0 32 32'>
...
var BxIcon = {
  ...
  AFK: virtual_controller_default,
...
var ENDING_CHUNKS_PATCH_NAME = "loadingEndingChunks", LOG_TAG3 = "Patcher", PATCHES = {
  disableAiTrack(str) {
    let index = str.indexOf(".track=function(");
    if (index < 0)
      return !1;
...
class SettingsNavigationDialog extends NavigationDialog {
  ...
  renderSettingRow(settingTab, $tabContent, settingTabContent, setting) {
    ...
    let label = prefDefinition?.label || setting.label, note = prefDefinition?.note || setting.note, experimental = prefDefinition?.experimental || setting.experimental;
...
class EmulatedMkbHandler extends MkbHandler {
  ...
  #initMessage = () => {
    if (!this.#$message)
      this.#$message = CE("div", { class: "bx-mkb-pointer-lock-msg bx-gone" }, CE("div", {}, CE("p", {}, t("virtual-controller")), CE("p", {}, t("press-key-to-toggle-mkb", { key: "F8" }))), CE("div", { "data-type": "virtual" }, createButton({
        ...
        label: "Away",
        onClick: (e) => {
          e.preventDefault(), e.stopPropagation(), this.toggleAwayMode(), this.waitForMouseData(!1);
        }
      }), createButton({
        label: t("edit"),
        style: 4,
  ...
  #initializeButtonLoop() {
    let buttonSequence = [
      { buttonIndex: 1, isKeyDown: !0 },
      { buttonIndex: 1, isKeyDown: !1 }
    ];
    this.#startButtonEmulationLoop(buttonSequence, 500);
  }
  #startButtonEmulationLoop(sequence, interval) {
    let currentIndex = 0, loop = () => {
      let { buttonIndex, isKeyDown } = sequence[currentIndex];
  ...
  #awayEnabled = !1;
  toggleAwayMode = (force) => {
    if (typeof force !== "undefined")
      this.#awayEnabled = force;
    else
      this.#awayEnabled = !this.#awayEnabled;
    if (this.#awayEnabled)
      this.startAwayMode(), this.#mouseDataProvider?.stop(), this.#initializeButtonLoop(), Toast.show("Away Mode", "Enabled");
    else
      this.stop(), this.#mouseDataProvider?.start(), Toast.show("Away Mode", "Disabled");
  };
  handleEvent(event) {
    switch (event.type) {
      case BxEvent.POINTER_LOCK_REQUESTED:
...
class ControllerShortcut {
  ...
  static #updateProfileList(e) {
    let $select = ControllerShortcut.#$selectProfile, $container = ControllerShortcut.#$container, $fragment = document.createDocumentFragment();
  ...
  static renderSettings() {
    ...
    let $remap, $selectProfile = CE("select", { class: "bx-shortcut-profile", autocomplete: "off" }), $profile = PREF_CONTROLLER_FRIENDLY_UI ? BxSelectElement.wrap($selectProfile) : $selectProfile, $container = CE("div", {
...
class RemotePlay {
  ...
  static #renderConsoles() {
    let $fragment = CE("div", { class: "bx-remote-play-container" });
    ...
    let $settingNote = CE("p", {}), resolutions = [1080, 720], currentResolution = getPref("xhome_resolution"), $resolutionGroup = CE("div", {});
    for (let resolution of resolutions) {
      let value = `${resolution}p`, id2 = `bx_radio_xhome_resolution_${resolution}`, $radio = CE("input", {
        type: "radio",
        value,
        id: id2,
    ...
    let $qualitySettings = CE("div", { class: "bx-remote-play-settings" }, CE("div", {}, CE("label", {}, t("target-resolution"), $settingNote), $resolutionGroup));
  ...
  static detachPopup() {
    let $popup = document.querySelector(".bx-remote-play-popup");
    $popup && $popup.remove();
  }
  static togglePopup(force = null) {
    ...
    let $header = document.querySelector("#gamepass-root header"), group = $header.firstElementChild.getAttribute("data-group");
...
class XhomeInterceptor {
  ...
  static async handle(request) {
    ...
    let deviceInfo = RemotePlay.BASE_DEVICE_INFO;
...
class LoadingScreen {
  ...
  static #hideRocket() {
    let $bgStyle = LoadingScreen.#$bgStyle, css = "#game-stream div[class*=RocketAnimation-module__container] > svg{display:none}#game-stream video[class*=RocketAnimationVideo-module__video]{display:none}";
...
class StreamBadges {
  ...
  async#updateBadges(forceUpdate = !1) {
    ...
    let now = +new Date, diffSeconds = Math.ceil((now - this.startTimestamp) / 1000), playtime = this.#secondsToHm(diffSeconds), batteryLevel = "100%", batteryLevelInt = 100, isCharging = !1;
    if (STATES.browser.capabilities.batteryApi)
      try {
        ...
        if (isCharging = bm.charging, batteryLevelInt = Math.round(bm.level * 100), batteryLevel = `${batteryLevelInt}%`, batteryLevelInt != this.startBatteryLevel) {
          let diffLevel = Math.round(batteryLevelInt - this.startBatteryLevel), sign = diffLevel > 0 ? "+" : "";
          batteryLevel += ` (${sign}${diffLevel}%)`;
        }
      } catch (e) {
      }
    let stats = await STATES.currentStream.peerConnection?.getStats(), totalIn = 0, totalOut = 0;
    stats.forEach((stat) => {
  ...
  #humanFileSize(size) {
    let units = ["B", "KB", "MB", "GB", "TB"], i = size == 0 ? 0 : Math.floor(Math.log(size) / Math.log(1024));
  ...
  async render() {
    ...
    if (this.#video) {
      if (video && (video += "/"), video += this.#video.codec, this.#video.profile) {
        let profile = this.#video.profile, quality = profile;
    ...
    if (this.#audio) {
      ...
      let bitrate = this.#audio.bitrate / 1000;
      audio += ` (${bitrate} kHz)`;
    }
    let batteryLevel = "";
    if (STATES.browser.capabilities.batteryApi)
    ...
    return BADGES.forEach((item2) => {
      if (!item2)
        return;
      let $badge = this.#renderBadge(...item2);
      $container.appendChild($badge);
    }), this.#$container = $container, await this.#start(), $container;
  }
  async#getServerStats() {
    let stats = await STATES.currentStream.peerConnection.getStats(), allVideoCodecs = {}, videoCodecId, allAudioCodecs = {}, audioCodecId, allCandidates = {}, candidateId;
  ...
  static setupEvents() {
    window.addEventListener(BxEvent.STREAM_PLAYING, (e) => {
      let $video = e.$video, streamBadges = StreamBadges.getInstance();
...
class WebGL2Player {
  ...
  drawFrame() {
    let gl = this.#gl, $video = this.#$video;
...
class TouchControlAction extends BaseGameBarAction {
  ...
  constructor() {
    ...
    let onClick = (e) => {
      ...
      let $parent = e.target.closest("div[data-enabled]"), enabled = $parent.getAttribute("data-enabled", "true") === "true";
...
class GameTile {
  ...
  static async#showWaitTime($elm, productId) {
    ...
    if (typeof totalWaitTime === "number" && isElementVisible($elm)) {
      let $div = CE("div", { class: "bx-game-tile-wait-time" }, createSvgIcon(BxIcon.PLAYTIME), CE("span", {}, GameTile.#secondsToHms(totalWaitTime)));
...
class StreamUiHandler {
  ...
  static async handleStreamMenu() {
    ...
    if (typeof $btnAwayMode === "undefined")
      $btnAwayMode = StreamUiHandler.cloneCloseButton($btnCloseHud, BxIcon.CONTROLLER, "bx-stream-controller-button", () => {
        EmulatedMkbHandler.getInstance().toggleAwayMode();
      });
    if ($btnRefresh && $btnHome && $btnAwayMode)
      $btnCloseHud.insertAdjacentElement("afterend", $btnRefresh), $btnRefresh.insertAdjacentElement("afterend", $btnHome), $btnCloseHud.insertAdjacentElement("afterend", $btnAwayMode);
  ...
  static handleSystemMenu($streamHud) {
    let $gripHandle = $streamHud.querySelector("button[class^=GripHandle]");
    if (!$gripHandle)
      return;
    ...
    let $btnAwayMode = StreamUiHandler.$btnAwayMode;
    if (typeof $btnAwayMode === "undefined")
      $btnAwayMode = StreamUiHandler.cloneStreamHudButton($orgButton, "Away Mode", BxIcon.VIRTUAL_CONTROLLER), $btnAwayMode?.addEventListener("click", (e) => {
        hideGripHandle(), e.preventDefault(), EmulatedMkbHandler.getInstance().toggleAwayMode();
    ...
    if ($btnStreamSettings && $btnStreamStats && $btnAwayMode) {
      let btnStreamStatsOn = !streamStats.isHidden() && !streamStats.isGlancing();
      $btnStreamStats.classList.toggle("bx-stream-menu-button-on", btnStreamStatsOn), $btnParent.insertBefore($btnStreamStats, $btnParent.lastElementChild), $btnParent.insertBefore($btnStreamSettings, $btnStreamStats), $btnParent.insertBefore($btnAwayMode, $btnStreamSettings);
    }
...
if (BX_FLAGS.SafariWorkaround && document.readyState !== "loading") {
  window.stop();
  let css = '.bx-reload-overlay{position:fixed;top:0;bottom:0;left:0;right:0;display:flex;align-items:center;background:rgba(0,0,0,0.8);z-index:9999;color:#fff;text-align:center;font-weight:400;font-family:"Segoe UI",Arial,Helvetica,sans-serif;font-size:1.3rem}.bx-reload-overlay *:focus{outline:none !important}.bx-reload-overlay > div{margin:0 auto}.bx-reload-overlay a{text-decoration:none;display:inline-block;background:#107c10;color:#fff;border-radius:4px;padding:6px}', isSafari = UserAgent.isSafari(), $secondaryAction;
  if (isSafari)
