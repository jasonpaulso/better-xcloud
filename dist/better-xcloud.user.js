...
// @grant        none
// @updateURL    https://github.com/jasonpaulso/better-xcloud/releases/latest/download/better-xcloud.meta.js
// @downloadURL  https://github.com/jasonpaulso/better-xcloud/releases/latest/download/better-xcloud.user.js
// ==/UserScript==
"use strict";
...
var SCRIPT_VERSION = "x.x.x", SCRIPT_VARIANT = "full", AppInterface = window.AppInterface;
UserAgent.init();
...
function checkForUpdate() {
 ...
 setPref("version_last_check", now), fetch("https://api.github.com/repos/jasonpaulso/better-xcloud/releases/latest").then((response) => response.json()).then((json) => {
  console.log("json", json), console.log("Latest version: ", json.tag_name), console.log("Current version: ", SCRIPT_VERSION), setPref("version_latest", json.tag_name), setPref("version_current", SCRIPT_VERSION);
...
class SoundShortcut {
 ...
 static muteUnmute() {
  if (getPref("audio_enable_volume_control") && STATES.currentStream.audioGainNode) {
   ...
   if (getPref("audio_volume") === 0 || gainValue === 0) SoundShortcut.unmute();
   else SoundShortcut.mute();
   return;
  }
  let $media;
 ...
 static mute(temporary = !1) {
  if (SoundShortcut.setGainNodeVolume(0), !temporary) setPref("audio_volume", 0, !0);
  Toast.show(`${t("stream")} ❯ ${t("volume")}`, t("muted"), { instant: !0 }), BxEvent.dispatch(window, BxEvent.SPEAKER_STATE_CHANGED, {
   speakerState: 1
  });
 }
 static unmute() {
  let settingValue = getPref("audio_volume"), targetValue = settingValue > 0 ? settingValue : 100;
  SoundShortcut.setGainNodeVolume(targetValue), Toast.show(`${t("stream")} ❯ ${t("volume")}`, targetValue + "%", { instant: !0 }), BxEvent.dispatch(window, BxEvent.SPEAKER_STATE_CHANGED, {
   speakerState: 0
  });
 }
 static getPrefVolume() {
  return getPref("audio_volume");
 }
}
...
class FO76AutomationHandler {
 ...
 observers = new Set;
 constructor() {
  this.mkbHandler = EmulatedMkbHandler.getInstance(), this.pressButton = this.mkbHandler.pressButton.bind(this.mkbHandler), this.onPointerLockExited = this.mkbHandler.onPointerLockExited.bind(this), this.buttonPressHandler = new ButtonPressHandler(this.pressButton), this.loopManager = new LoopManager;
 }
 static getInstance() {
  if (!this.instance) this.instance = new FO76AutomationHandler;
 ...
 init = () => {
  console.log("FO76AutomationHandler", "Initializing game automation handler"), EventManager.registerEventListeners(this), this.initializeDefaultConfigs();
 };
 get enabled() {
 ...
 set windowFocused(value) {
  this._windowFocused = value;
 }
 DEFAULT_ACTION_INTERVAL = 1000;
  ...
  [
   {
    ...
    action: async () => {
     this.pressButton(15, !0), await this.delay(500), this.pressButton(15, !1);
    }
   },
   {
    name: "pivot",
    ...
    action: async () => {
     await this.pressButtonWithRandomDelay(203, this.DEFAULT_ACTION_INTERVAL), await this.delay(500), await this.pressButtonWithRandomDelay(202, this.DEFAULT_ACTION_INTERVAL);
    }
   },
   {
    name: "automation",
    ...
    pauseDuration: this.PIVOT_PAUSE_DURATION,
    action: async () => {
     this.pressButton(10, !0), await this.delay(500), this.pressButton(10, !1);
    }
   },
   {
    name: "vats",
    ...
    pauseDuration: this.DEFAULT_PAUSE_DURATION,
    action: async () => {
     this.pressButton(4, !0), await this.delay(500), this.pressButton(4, !1), this.pressButton(7, !0), await this.delay(this.VATS_PAUSE_DURATION), this.pressButton(7, !1);
    }
   }
  ].forEach((config) => this.updateMode(config.name, config));
 }
 updateMode(modeName, config) {
  if (this.stopButtonLoop(modeName), this.loopConfigs.set(modeName, {
   ...config,
   name: modeName,
   isRunning: !1,
 ...
 startButtonLoop(mode) {
  Toast.show("Game FO76Automation", `Starting ${mode} loop`, { instant: !0 }), this.setButtonLoop(mode, !0);
 }
 stopButtonLoop(mode) {
  Toast.show("Game FO76Automation", `Stopping ${mode} loop`, { instant: !0 }), this.setButtonLoop(mode, !1);
 }
 setButtonLoop(mode, shouldStart) {
  let config = this.loopConfigs.get(mode);
  if (!config) return;
  if (shouldStart) this.loopManager.startLoop(config, mode, this);
  else this.loopManager.stopLoop(config, mode);
  this.notifyObservers();
 }
 startCountdown(mode) {
  let config = this.loopConfigs.get(mode);
  if (!config) return;
  let countdown = (config.actionInterval + config.pauseDuration) / 1000, logIntervalId = setInterval(() => {
   if (config.isRunning) {
    if (console.log("FO76AutomationHandler", `Loop ${mode} will run again in ${countdown} seconds`), Toast.show("Game FO76Automation", `${mode} will run again in ${countdown} seconds`, {
     instant: !0
    }), countdown--, countdown < 0)
     countdown = (config.actionInterval + config.pauseDuration) / 1000;
   } else clearInterval(logIntervalId);
  }, 1000);
 }
 toggle = () => {
  console.log("FO76AutomationHandler", "Toggling game automation"), !this.enabled ? this.activate() : this.deactivate();
 };
 activate = () => {
  if (this.enabled) {
   BxLogger.warning("FO76AutomationHandler", "Game automation is already activated");
   return;
  }
  console.log("FO76AutomationHandler", "Activating game automation"), this.enabled = !0, this.startFO76Automation(), this.notifyObservers();
 };
 deactivate = () => {
  if (!this.enabled) {
   BxLogger.warning("FO76AutomationHandler", "Game automation is already deactivated");
   return;
  }
  console.log("FO76AutomationHandler", "Deactivating game automation"), this.enabled = !1, this.stopFO76Automation(), this.notifyObservers();
 };
 startFO76Automation() {
  this.startMkbHandler(), this.showActivationMessage();
 }
 stopFO76Automation() {
  this.stopAllLoops(), this.stopMkbHandler(), this.showDeactivationMessage();
 }
 startMkbHandler() {
  this.mkbHandler.start(), this.onPointerLockExited(), this.mkbHandler.hideMessage();
 }
 stopMkbHandler() {
  this.mkbHandler.stop(), this.mkbHandler.hideMessage();
 }
 showActivationMessage() {
  Toast.show("Game FO76Automation", "Activated", { instant: !0 });
 }
 showDeactivationMessage() {
  Toast.show("Game FO76Automation", "Deactivated", { instant: !0 });
 }
 destroy = () => {
  this.enabled = !1, this.stopAllLoops(), this.mkbHandler.destroy();
 };
 stopAllLoops() {
  for (let [mode] of this.loopConfigs)
   this.stopButtonLoop(mode);
 }
 async pressButtonWithRandomDelay(buttonCode, maxDelay) {
  await this.buttonPressHandler.pressButtonWithRandomDelay(buttonCode, maxDelay);
 }
 async delay(ms) {
  await TimingUtils.delay(ms);
 }
 toggleGamepads() {
  window.navigator.getGamepads = () => !this.windowFocused ? [] : this.mkbHandler.patchedGetGamepads();
 }
 handleWindowBlur = () => {
  window.navigator.getGamepads = () => this.mkbHandler.getVirtualGamepads(), SoundShortcut.mute(!0), console.log("FO76AutomationHandler", "Window blurred");
 };
 handleWindowFocus = () => {
  if (window.navigator.getGamepads = () => this.mkbHandler.patchedGetGamepads(), SoundShortcut.getPrefVolume() > 0) SoundShortcut.unmute();
 ...
 handleToggleFO76AutomationEvent = async (event) => {
  let customEvent = event;
  if (this.toggle(), console.log("FO76AutomationHandler", "Toggle automation event (FO76_AUTOMATION_EVENTS.TOGGLE_AUTOMATION): ", customEvent), console.log("FO76AutomationHandler", "Toggle automation event: ", event), customEvent.detail && typeof customEvent.detail.callback === "function") customEvent.detail.callback({
    status: "success",
    automationActive: this.enabled
   });
 };
 handleToggleModeEvent = async (event) => {
  let customEvent = event;
  console.log("FO76AutomationHandler", "Toggle mode event (FO76_AUTOMATION_EVENTS.TOGGLE_MODE): ", event);
  let mode = customEvent.detail, response = await this.handleFO76AutomationEvent({
   automationMode: mode
  });
  if (customEvent.detail && typeof customEvent.detail.callback === "function") customEvent.detail.callback(response);
 };
 isConfigChanged(mode, existingConfig) {
  let changeLog = {
   actionInterval: {
  ...
  console.log("FO76AutomationHandler", `Change log for ${mode.name}: `, changeLog);
  let isConfigChanged = Object.values(changeLog).some((value) => {
   return value.changed;
  });
  return console.log("FO76AutomationHandler", `Is config changed for ${mode.name}: `, isConfigChanged), isConfigChanged;
 }
 handleFO76AutomationEvent = async (state) => {
  if (state.automationMode) this.activate();
  else this.deactivate();
  try {
   let mode = state.automationMode;
   if (mode && mode.name) {
    let existingConfig = this.loopConfigs.get(mode.name);
    if (mode.toggle) {
     if (existingConfig) if (this.updateMode(mode.name, {
      ...
      else this.stopButtonLoop(mode.name);
     return {
      status: "success",
      message: "FO76Automation event handled",
      mode,
      modeStatus: mode.name ? this.loopConfigs.get(mode.name) : void 0
     };
    }
    if (!existingConfig || !existingConfig.isRunning) {
     if (console.log("FO76AutomationHandler", `Starting mode ${mode.name}`), this.updateMode(mode.name, {
      ...
      this.startButtonLoop(mode.name);
    } else if (console.log("FO76AutomationHandler", `Stopping mode ${mode.name}`), !this.isConfigChanged(mode, existingConfig)) this.stopButtonLoop(mode.name);
    else if (console.log("FO76AutomationHandler", `Updating mode ${mode.name}`), this.updateMode(mode.name, {
     actionInterval: mode.interval || existingConfig.actionInterval,
     pauseDuration: mode.pause || existingConfig.pauseDuration,
     action: existingConfig.action,
     enabled: mode.enabled
     ...
     this.startButtonLoop(mode.name);
   } else console.log("FO76AutomationHandler", "No mode provided, stopping all modes");
   return {
    status: "success",
    message: "FO76Automation event handled",
    mode,
   ...
   return BxLogger.error("FO76AutomationHandler", "Failed to handle automation event", error), {
    status: "error",
    message: "Failed to handle automation event"
   };
  }
 };
 toggleFO76Automation = async (_toggle) => {
  try {
   return this.toggle(), {
    status: "success",
    automationActive: this.enabled
   ...
   return BxLogger.error("FO76AutomationHandler", "Failed to toggle automation", error), {
    status: "error",
    automationActive: this.enabled
   };
  }
 };
 sendDefaultEvent = async (modeName, interval, pause) => {
  try {
   let mode = {
    name: modeName,
    interval,
    pause,
    enabled: !0
   }, existingConfig = this.loopConfigs.get(mode.name);
   mode.enabled = !existingConfig?.isRunning, this.updateMode(mode.name, {
    actionInterval: mode.interval || existingConfig?.actionInterval || 1000,
   ...
   BxLogger.error("FO76AutomationHandler", `Failed to send default event for mode ${modeName}`, error);
  }
 };
 incrementDecrementDefaultInterval = async (increment, decrement) => {
  let newInterval = this.DEFAULT_ACTION_INTERVAL + (increment ? 1000 : decrement ? -1000 : 0);
  if (newInterval < 1000) {
   this.DEFAULT_ACTION_INTERVAL = 1000;
   return;
  }
  if (newInterval > 1e4) {
   this.DEFAULT_ACTION_INTERVAL = 1e4;
   return;
  }
  console.log("FO76AutomationHandler", `Incrementing default interval to ${newInterval}`), Toast.show("Game FO76Automation", `${increment ? "Increasing" : "Decreasing"} default interval to ${newInterval / 1000} seconds`, {
   instant: !0
  }), this.DEFAULT_ACTION_INTERVAL = newInterval;
  for (let [modeName, mode] of this.loopConfigs)
 ...
 handleIncrementDecrementDefaultInterval = async (event) => {
  let customEvent = event, increment = customEvent.detail.increment, decrement = customEvent.detail.decrement;
  return await this.incrementDecrementDefaultInterval(increment, decrement), {
   status: "success",
   automationActive: this.enabled
  };
 };
...
class EventManager {
 static registerEventListeners(handler) {
  setInterval(() => {
   if (document.hasFocus()) BxEvent.dispatch(window, "window-focused");
   else BxEvent.dispatch(window, "window-blurred");
  }, 200), window.addEventListener("blur", handler.handleWindowBlur), window.addEventListener("focus", handler.handleWindowFocus), window.addEventListener(BxEvent.STREAM_STOPPED, handler.handleStreamStopped), window.addEventListener("keydown", handler.handleKeyDown), window.addEventListener(FO76_AUTOMATION_EVENTS.TOGGLE_AUTOMATION, handler.handleToggleFO76AutomationEvent), window.addEventListener(FO76_AUTOMATION_EVENTS.TOGGLE_MODE, handler.handleToggleModeEvent), window.addEventListener(FO76_AUTOMATION_EVENTS.CHANGE_INTERVAL, handler.handleIncrementDecrementDefaultInterval);
 }
}
class TimingUtils {
 static async randomDelay(maxMs) {
  let delay = Math.random() * maxMs;
...
class ButtonPressHandler {
 pressButton;
 constructor(pressButton) {
  this.pressButton = pressButton;
 }
 async pressButtonWithRandomDelay(buttonCode, maxDelay) {
  this.pressButton(buttonCode, !0), await TimingUtils.randomDelay(maxDelay), this.pressButton(buttonCode, !1);
 }
}
class LoopManager {
 intervals = new Map;
 startLoop(config, mode, handler) {
  config.isRunning = !0;
  let loopFunction = async () => {
   if (!config.isRunning) return;
   if (console.log("FO76AutomationHandler", `Running ${mode} loop`), await config.action(), await TimingUtils.delay(config.pauseDuration), config.isRunning) this.intervals.set(mode, window.setTimeout(loopFunction, config.actionInterval));
  };
  loopFunction(), handler.startCountdown(mode);
 }
 stopLoop(config, mode) {
  let intervalId = this.intervals.get(mode);
  if (intervalId) clearTimeout(intervalId), this.intervals.delete(mode);
  config.isRunning = !1;
 }
}
 ...
 static runAction(action) {
  switch (action) {
   ...
   case "stream-volume-dec":
    ...
    break;
   case "toggle-away":
    BxEvent.dispatch(window, FO76_AUTOMATION_EVENTS.TOGGLE_AUTOMATION, {
     detail: { source: "controller" }
    });
    break;
   case "toggle-away-mode-heal":
    BxEvent.dispatch(window, FO76_AUTOMATION_EVENTS.TOGGLE_MODE, {
     detail: { name: "heal", toggle: !0 }
    });
    break;
   case "toggle-away-mode-pivot":
    BxEvent.dispatch(window, FO76_AUTOMATION_EVENTS.TOGGLE_MODE, {
     detail: { name: "pivot", toggle: !0 }
    });
    break;
   case "toggle-away-mode-away":
    BxEvent.dispatch(window, FO76_AUTOMATION_EVENTS.TOGGLE_MODE, {
     detail: { name: "away", toggle: !0 }
    });
    break;
   case "toggle-away-mode-vats":
    BxEvent.dispatch(window, FO76_AUTOMATION_EVENTS.TOGGLE_MODE, {
     detail: { name: "vats", toggle: !0 }
    });
    break;
   case "increment-away-mode-interval":
    BxEvent.dispatch(window, FO76_AUTOMATION_EVENTS.CHANGE_INTERVAL, {
     detail: { increment: !0, source: "controller" }
    });
    break;
   case "decrement-away-mode-interval":
    BxEvent.dispatch(window, FO76_AUTOMATION_EVENTS.CHANGE_INTERVAL, {
     detail: { decrement: !0, source: "controller" }
    });
    break;
 ...
 static renderSettings() {
  ...
  let actions = {
   [t("better-xcloud")]: {
    ...
    "toggle-away": ["Away Mode - Toggle", t("toggle")],
    "toggle-away-mode-heal": ["Away Mode — Heal", t("toggle")],
    "toggle-away-mode-pivot": ["Away Mode — Pivot", t("toggle")],
    "toggle-away-mode-away": ["Away Mode — Away", t("toggle")],
    "toggle-away-mode-vats": ["Away Mode — Vats", t("toggle")],
    "increment-away-mode-interval": ["Away Mode - Interval", t("increase")],
    "decrement-away-mode-interval": ["Away Mode - Interval", t("decrease")]
   },
   ...
   [t("stream")]: {
    ...
    "stream-volume-inc": getPref("audio_enable_volume_control") && [
     t("volume"),
     t("increase")
    ],
    "stream-volume-dec": getPref("audio_enable_volume_control") && [
     t("volume"),
     t("decrease")
    ],
  ...
  let $remap, $selectProfile = CE("select", {
   class: "bx-shortcut-profile",
   autocomplete: "off"
  }), $profile = PREF_CONTROLLER_FRIENDLY_UI ? BxSelectElement.wrap($selectProfile) : $selectProfile;
